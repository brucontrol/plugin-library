(function () {
  ...
  var sectionToggle = { value: "showDuty", interval: "showInterval" };
  var sectionPrefix = { value: "duty", interval: "interval" };
  ...
  function renderContentRows(type) {
    switch (type) {
      case "dutyCycle":
        var p = Math.max(0, Math.min(6, Math.floor(toNumber(currentData.precision, 1))));
        appendRow(primaryRow("Duty", toNumber(currentData.dutyCycle, 0).toFixed(p) + "%", "", "value"));
        appendRow(row("Interval", formatInterval(currentData.interval), "", { key: "interval" }));
        break;
      default:
  ...
  function getPreviewData() {
    var t = getType(null);
    var map = {
      dutyCycle: { elementType: "dutyCycle", name: "Duty Cycle", displayName: "Duty Cycle", dutyCycle: 37.5, interval: 1000, state: true, userControl: true, enabled: true, deviceConnected: true }
    };
