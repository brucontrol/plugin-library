(function () {
  ...
  function applyStyles() {
    ...
    labelNodes.forEach(function (node) {
      ...
      el.style.color = (pfx && d[pfx + "LabelColor"] && String(d[pfx + "LabelColor"]).trim()) ? String(d[pfx + "LabelColor"]).trim() : "";
    ...
    valueNodes.forEach(function (node) {
      ...
      var key = rowEl ? rowEl.getAttribute("data-row-key") : "";
      var pfx = sectionPrefix[key] || "";
      el.style.color = (pfx && d[pfx + "Color"] && String(d[pfx + "Color"]).trim()) ? String(d[pfx + "Color"]).trim() : "var(--accent-green, #4ec9b0)";
