(function () {
  var fill = document.getElementById('liquid-fill');
  var bubbleContainer = document.getElementById('bubbles');
  var stirLayer = document.getElementById('stir-layer');
  var bubbles = [];
  var animId = null;
  var lastSpawn = 0;
  ...
  function applyLiquidStyle(color) {
    if (!fill) return;
    var c = color || '#3b82f6';
  ...
  function applyStirLayer(color) {
    if (!stirLayer) return;
    var c = color || '#3b82f6';
    var rgb = hexToRgb(c);
    stirLayer.style.background =
      'conic-gradient(from 0deg at 50% 50%,' +
      'transparent 0deg,' +
      'rgba(' + rgb.r + ',' + rgb.g + ',' + rgb.b + ',0.15) 90deg,' +
      'transparent 180deg,' +
      'rgba(' + rgb.r + ',' + rgb.g + ',' + rgb.b + ',0.08) 270deg,' +
      'transparent 360deg)';
  }
  ...
  function render(data) {
    ...
    config.stirEnabled = data.stirEnabled === true || data.stirEnabled === 'true';
    config.stirSpeedMs = parseInt(data.stirSpeedMs, 10);
    if (isNaN(config.stirSpeedMs) || config.stirSpeedMs < 500) config.stirSpeedMs = 4000;
    ...
    applyLiquidStyle(config.liquidColor);
    applyStirLayer(config.liquidColor);

    document.documentElement.style.setProperty('--wave-duration', '2.8s');
    document.documentElement.style.setProperty('--stir-duration', (config.stirSpeedMs / 1000) + 's');

    if (stirLayer) {
      stirLayer.classList.toggle('active', config.stirEnabled && pct > 2);
    }
