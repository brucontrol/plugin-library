(function () {
  ...
  function renderContentRows(type) {
    switch (type) {
      case "deadband":
        var prec = Math.max(0, Math.min(6, Math.round(toNumber(currentData.precision, 2))));
        var d = currentData || {};
        ...
        appendRow(row("Band", formatCalibratedReading(d, toNumber(d.band || d.deadbandOffset, 0).toFixed(prec)), "", { key: "band" }));
        break;
      default:
  ...
  function getPreviewData() {
    var t = getType(null);
    var map = {
      deadband: { elementType: "deadband", name: "Deadband", displayName: "Deadband", target: 66, value: 50, deadbandOffset: 2, precision: 2, userControl: true, enabled: true, deviceConnected: true, inputDisplayName: "Temp Probe", inputElementId: "", inputElementType: "",
