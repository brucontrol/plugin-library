(function () {
  ...
  function applyCssVars() {
    ...
    widget.style.setProperty("--tube-color", state.tubeColor);
    widget.style.setProperty("--fluid-color", state.fluidColor);
    widget.style.setProperty("--tube-width", t + "px");
    widget.style.setProperty("--fluid-width", Math.round(t * 0.65) + "px");
    widget.style.setProperty("--hit-width", (t + 10) + "px");
  }
  ...
  function render() {
    ...
    widget.classList.toggle("has-bends", state.bendPoints.length > 0);
    widget.classList.toggle("handles-on", state.showHandles);
    widget.classList.toggle("reset-on", state.showResetBtn);
    applyCssVars();
  }
  ...
  function setFocused(val) {
    state.focused = val;
    widget.classList.toggle("focused", val);
  }

  widget.addEventListener("pointerdown", function () { setFocused(true); });
  document.addEventListener("pointerdown", function (e) {
    if (!widget.contains(e.target)) setFocused(false);
  });
  ...
  function animateFrame() {
    ...
    var dashCycle = 6 + 14;
    widget.style.setProperty("--fluid-offset", String(animOffset % (dashCycle * 4)));
