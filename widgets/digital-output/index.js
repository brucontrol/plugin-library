(function () {
  ...
  function renderContentRows(type, hiddenRows) {
    switch (type) {
      case "digitalOutput":
        appendRow(primaryRow("Output", boolText(asBool(currentData.state)), asBool(currentData.state) ? "value--ok" : "value--warn", "output", hiddenRows));
    ...
    switch (type) {
      case "digitalOutput":
        footerEl.appendChild(
          makeButton(asBool(currentData.state) ? "Turn OFF" : "Turn ON", function () {
  ...
  function getPreviewData() {
    var t = getType(null);
    var map = {
      digitalOutput: { elementType: "digitalOutput", name: "Digital Out", displayName: "Digital Out", state: true, activeLow: false, userControl: true, enabled: true, deviceConnected: true }
    };
